...
import com.forwardmeasure.openworkflow.binding.micronaut.MicronautActiveOrganizationProvider;
...
public class MicronautExecutionController {
  ...
  @Post("/{id}/pause")
  HttpResponse<Object> pause(
      @PathVariable UUID id,
      @Header("If-Match") String ifMatch,
      @Header("X-Correlation-ID") String correlation,
      @Body ExecutionControl request,
      Authentication authentication) {
    return inTenant(authentication, () -> delegate.pause(id, ifMatch, correlation, request));
  }

  @Post("/{id}/resume")
  HttpResponse<Object> resume(
      @PathVariable UUID id,
      @Header("If-Match") String ifMatch,
      @Header("X-Correlation-ID") String correlation,
      @Body ExecutionControl request,
      Authentication authentication) {
    return inTenant(authentication, () -> delegate.resume(id, ifMatch, correlation, request));
  }

  @Post("/{id}/cancel")
  HttpResponse<Object> cancel(
      @PathVariable UUID id,
      @Header("If-Match") String ifMatch,
      @Header("X-Correlation-ID") String correlation,
      @Body ExecutionControl request,
      Authentication authentication) {
    return inTenant(authentication, () -> delegate.cancel(id, ifMatch, correlation, request));
  }
