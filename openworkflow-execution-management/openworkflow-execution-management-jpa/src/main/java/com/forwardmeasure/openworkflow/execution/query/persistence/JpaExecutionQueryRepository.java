...
public final class JpaExecutionQueryRepository implements ExecutionQueryRepository {
  private static final String PROJECTION_SELECT =
      "select"
          ...
          + "e.created_at,e.updated_at,e.completed_at,e.input,p.output,p.error,coalesce(p.effects,'[]'::jsonb),coalesce(p.timers,'[]'::jsonb),w.uuid"
          + " from ";
  ...
  private String projectionSql() {
    return PROJECTION_SELECT
        ...
        + schema
        + ".workflow w on w.id=r.workflow_id left join "
        + schema
