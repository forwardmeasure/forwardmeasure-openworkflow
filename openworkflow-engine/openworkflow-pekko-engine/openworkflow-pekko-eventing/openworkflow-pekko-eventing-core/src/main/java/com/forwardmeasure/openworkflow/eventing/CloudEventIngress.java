...
/** Persist-confirmed inbound CloudEvent delivery port. */
public interface CloudEventIngress {
  CompletionStage<WorkflowReply> deliver(
      ExecutionId executionId, WorkflowCloudEvent event, Instant receivedAt);

  CompletionStage<ScheduleReply> deliver(
      ScheduleId scheduleId, WorkflowCloudEvent event, Instant receivedAt);

  CompletionStage<CloudEventRouteResult> route(
