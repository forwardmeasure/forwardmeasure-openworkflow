...
/** Repartitions subscription effects by tenant for co-located event routing. */
final class OksSubscriptionEffectOutputProcessor
  ...
  public void process(Record<String, WorkflowEffect> record) {
    ...
    if (effect == null
        || (effect.type() != WorkflowEffectType.UPSERT_EVENT_SUBSCRIPTION
            && effect.type() != WorkflowEffectType.DELETE_EVENT_SUBSCRIPTION)) {
      return;
    }
    context().forward(record.withKey(effect.key().tenantId().toString()));
  }
}
/*
