...
/** Materializes immutable execution history under sequence-ordered keys. */
final class OksHistoryQueryProcessor
    extends ContextualProcessor<String, ExecutionHistoryEvent, Void, Void> {
  private KeyValueStore<String, ExecutionHistoryEvent> store;

  @Override
  public void init(org.apache.kafka.streams.processor.api.ProcessorContext<Void, Void> context) {
    super.init(context);
    store = context.getStateStore(OksStores.HISTORY);
  }

  @Override
  public void process(Record<String, ExecutionHistoryEvent> record) {
    if (record.value() == null) return;
    if (record.value().type() == ExecutionEventType.EXECUTION_PURGED) {
      deleteExecution(record.value().key().canonical());
    }
    store.put(
        OksQueryKeys.history(record.value().key(), record.value().sequence()), record.value());
  }

  private void deleteExecution(String canonicalKey) {
    java.util.ArrayList<String> keys = new java.util.ArrayList<>();
    try (org.apache.kafka.streams.state.KeyValueIterator<String, ExecutionHistoryEvent> values =
