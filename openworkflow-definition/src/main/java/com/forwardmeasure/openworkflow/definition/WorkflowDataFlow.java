...
/** Top-level workflow validation and transformation contract. */
public record WorkflowDataFlow(
    ResolvedDataSchema inputSchema,
    JsonNode inputFrom,
    JsonNode outputAs,
    ResolvedDataSchema outputSchema) {

  public WorkflowDataFlow {
    inputFrom = copy(inputFrom);
    outputAs = copy(outputAs);
  }

  public static WorkflowDataFlow identity() {
    return new WorkflowDataFlow(null, null, null, null);
  }
