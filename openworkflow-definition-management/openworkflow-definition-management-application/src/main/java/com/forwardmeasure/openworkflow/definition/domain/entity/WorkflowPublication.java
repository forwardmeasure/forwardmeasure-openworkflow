...
/** The single publication (and, later, deprecation) record for one definition. */
@Entity
@Table(name = "workflow_publication")
@Getter
@NoArgsConstructor(access = AccessLevel.PROTECTED)
public class WorkflowPublication extends AbstractBaseEntity<Long> {
  @Serial private static final long serialVersionUID = 1L;
  private static final Pattern SHA_256 = Pattern.compile("[0-9a-f]{64}");

  @Id
  @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "workflow_publication_ids")
  @SequenceGenerator(
      name = "workflow_publication_ids",
      sequenceName = "workflow_publication_id_seq",
      allocationSize = 50)
  @Setter
  private Long id;
  ...
  public WorkflowPublication(WorkflowDefinition definition, Actor actor, String definitionDigest) {
  ...
  public void deprecate() {
    if (deprecatedAt != null) {
      throw new IllegalStateException("Publication is already deprecated");
    }
    deprecatedAt = OffsetDateTime.now();
  }
